...
class DM(object):
    ...
    def run(self, cmdline, logfile=None,
        ...
        if name:
            ...
            if dm_path:
                dm = Daemon.load(dm_path)
                if dm and dm.is_alive():
                    print('simSpark Daemon is already running.')
                    return
        else:
            dm_path = None
    ...
    def kill(self, name=None, group=None, quiet=False, sigkill=False):
        ...
        if len(daemons) > 0:
            notify = 'Stopping simSpark Daemon (%d)\n' % len(daemons)
            if quiet is False:
    ...
    def restart(self, name=None, group=None, cmd=None,
        ...
        daemons = self.get_daemons(name, group)
        if len(daemons) > 0:
            notify = 'Restarting simSpark daemon (%d)' % len(daemons)
            if quiet is False:
                notify += 'are you sure? [Y/n]'
                yn = input(notify)
            else:
...
def help():
    print("simSpark Daemon Process\n"
          "Usage: %s {start|stop|restart|status|transcript|help} [args_to_simSpark]\n\n" % sys.argv[0] +
          "simSpark help:")
    subprocess.call([sys.executable, "master_body.py", "-h"])
...
def main():
    transcript_path = "simSpark_daemon_manager.log"
    instance_name = str(crc32(os.path.dirname(os.path.abspath(inspect.stack()[0][1])).encode()))
    if len(sys.argv) < 2:
        help()
        exit()
    dm = DM()
    simSpark_args = " ".join(sys.argv[2:])
    if len(dm.get_daemons(name="simSpark")):
        print("Old daemon process is killed.")
        dm.kill(name="simSpark", quiet=True, sigkill=True)
    if sys.argv[1] == "start":
        dm.run(cmdline=" ".join((sys.executable + " master_body.py", simSpark_args)),
               name=instance_name,
        ...
        if len(sys.argv) > 2:
            kwargs["cmd"] = " ".join((sys.executable, "master_body.py", simSpark_args))
        try:
            dm.restart(**kwargs)
